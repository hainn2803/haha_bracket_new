...
def evaluate_handles(ctx, bank, handles, downstream_handle=None, r_handle=None, restore_handle=None):
    # Certify candidate handles on the given Dcal or Dte bank.
    if downstream_handle is None:
        ...
        for i, handle in enumerate(handles):
            ...
            results.append({**handle, "summary": summary, "sensitivity_score": sensitivity, "invariance_score": invariance, "restoration": None, "passed": bool(summary["all_rates_at_least_0_90"])})
        return results
    ...
    for i, handle in enumerate(handles):
        ...
        sensitivity, invariance = get_block_scores(summary)
        passed = bool(summary["passes"] and restoration["passes"] and sensitivity >= ctx.args.sensitivity_threshold and invariance >= ctx.args.invariance_threshold)
        results.append({**handle, "summary": summary, "sensitivity_score": sensitivity, "invariance_score": invariance, "restoration": restoration, "passed": passed})
    return results


def get_valid_handles(handles, variable):
    # Keep Dcal-passing handles that represent the requested variable.
    if variable == "R":
        return [row for row in handles if row["passed"] and not row["is_D"] and min(row["r_accuracy"].values()) >= 0.9]
...
def refine_handles(ctx, cal_bank, valid_handles, late_handle, r_handle=None):
    # Repeatedly find the strongest passing handle upstream of the current frozen handle.
...
def print_refinement(variable, chain, rounds):
    # Print the selected chain and the strongest candidates tested in each refinement round.
    print(f"{variable} chain:")
    for row in chain:
        print(row["name"], row["site_ids"], row["strength"], handle_order(row))

    print(f"{variable} refinement rounds:", len(rounds))
    for i, round_result in enumerate(rounds, 1):
        results = round_result["results"]
        passed = [row for row in results if row["passed"]]
        print(f"Round {i}: downstream={round_result['downstream_handle']['site_ids']}, tested={len(results)}, passed={len(passed)}")
        ...
        for row in ranked[:10]:
            print({
                ...
                "summary_pass": row["summary"]["passes"],
                "sensitivity": row["sensitivity_score"],
                "invariance": row["invariance_score"],
                "restoration_pass": row["restoration"]["passes"],
...
def main():
    ...
    print("R chain:")
    for row in r_chain:
        print(row["name"], row["site_ids"], handle_order(row))

    print("Frozen R for D discovery:", final_r["site_ids"], handle_order(final_r))
    ##############################################
    ...
    d_valid_handles = get_valid_handles(d_cal_results, "D")

    ##############################################
    print("D candidate handles:", len(d_candidate_handles))
    print("D calibration results:", len(d_cal_results))
    print("D valid handles:", len(d_valid_handles))
    print("Frozen R:", final_r["site_ids"], final_r["strength"])
    for row in sorted(d_cal_results, key=lambda row: float(row["summary"]["score"]), reverse=True)[:10]:
        print({
            "sites": row["site_ids"],
            "strength": row["strength"],
            "is_D": row["is_D"],
    ...
    assert d_valid_handles, "No D handle passed Dcal certification"
    ##############################################
