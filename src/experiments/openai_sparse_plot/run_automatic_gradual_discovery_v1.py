...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-r-refinements", type=int, default=3)
...
def handle_order(weights: Mapping[str, float]) -> tuple[int, int]:
    return min(layer_order(site_id) for site_id in weights)
...
# Stage 2: freeze R_late and search for an earlier R.
def refine_r_once(ctx: Context, coarse: Data, graded: Data, current: Mapping[str, Any], parent_support: Sequence[Mapping[str, Any]], strengths: Sequence[float], round_index: int) -> dict[str, Any] | None:
    ...
    if not earlier:
        return None
    print(f"[2] R refinement {round_index}: freeze {list(current['weights'])}", flush=True)
    ...
    # Dcal: select handles that move frozen R and Y correctly.
    configs, metadata = make_calibration_configs(support, strengths)
    ...
    # Dte: validate directionality and restoration of frozen R.
    test_configs = tuple(HandleConfiguration(row["handle_id"], row["weights"], row["strength"]) for row in tier)
    ...
    return {"round": round_index, "frozen_downstream": public_handle(current), "candidate_ids": [site.site_id for site in candidates], "normalization_scales": scales, "selector": selector, "effective_support": support, "calibration_best": best, "calibration_best_tier": tier, "heldout_tier": heldout, "classified": classified}
...
# Stage 3: freeze D_late and search for an earlier D.
def refine_d_once(ctx: Context, graded: Data, current: Mapping[str, Any], r_weights: Mapping[str, float], r_readout: Any, strengths: Sequence[float], round_index: int) -> dict[str, Any] | None:
    ...
    if not candidates:
        return None
    print(f"[3] D refinement {round_index}: freeze {list(d_weights)}", flush=True)
...
def main() -> None:
    ...
    support = coarse_result["effective_support"]
    for round_index in range(args.max_r_refinements):
        stage = refine_r_once(ctx, coarse, graded, r_current, support, strengths, round_index)
        if stage is None:
            break
        atomic_json(args.out_dir / f"R_refinement_{round_index}.json", {key: value for key, value in stage.items() if key != "classified"})
        d_candidate = choose_d(stage["classified"])
        if d_candidate is not None:
            d_current = d_candidate
            break
        r_candidate = choose_r(stage["classified"], rank)
        if r_candidate is None:
            break
        r_current = r_candidate
        support = stage["effective_support"]
        r_chain.append({"name": "R_early", **public_handle(r_current), "source": f"R refinement {round_index}"})

    if d_current is None:
        raise RuntimeError("no graded D handle emerged from the coarse/R support")
    direct_d = validate_d(ctx, graded, d_current, r_current)
    r_weights = r_current["weights"]
    r_readout = direct_d["R_readout"]
    d_chain = [{"name": "D", **public_handle(d_current), "source": "coarse/R refinement"}]
