...
def calibrate_against_frozen_r(ctx: Context, data: Data, ranked_pool: Sequence[Mapping[str, Any]], frozen_r: Mapping[str, Any], strengths: Sequence[float]) -> dict[str, Any]:
    probes = get_probe_sites(ctx, frozen_r["weights"])
    probe_ids = tuple(site.site_id for site in probes)
...
def rank_against_frozen_d(ctx: Context, data: Data, candidates: Sequence[ChannelSite], frozen_d: Mapping[str, Any]) -> dict[str, Any]:
    ...
    d_decoder, _ = fit_d(data.examples, data.runs, d_weights)
...
def calibrate_against_frozen_d(ctx: Context, data: Data, ranked_pool: Sequence[Mapping[str, Any]], frozen_d: Mapping[str, Any], r_weights: Mapping[str, float], r_readout: Any, strengths: Sequence[float]) -> dict[str, Any]:
    d_weights = frozen_d["weights"]
    d_decoder, _ = fit_d(data.examples, data.runs, d_weights)
    ...
    test_configs = tuple(HandleConfiguration(row["handle_id"], row["weights"], row["strength"]) for row in tier)
    test_margins, test_probes = run_probes(ctx, data, test_configs, "Dte", probes)
    restored_margins, _ = run_probes(ctx, data, test_configs, "Dte", probes, tuple(d_weights))
    heldout = []
    for index, row in enumerate(tier):
        summary = d_summary(data, "Dte", test_margins[index], test_probes[index], probe_ids, d_weights, d_decoder, r_weights, r_readout)
        mediation = mediation_summary(data.pairs["Dte"], data.by_id, data.runs, test_margins[index], restored_margins[index])
        heldout.append({**row, "heldout": summary, "mediation": mediation, "accepted": is_certified(summary, mediation)})
    return {"calibration_best": best, "calibration_best_tier": tier, "heldout": heldout, "accepted": [row for row in heldout if row["accepted"]]}
...
def main() -> None:
    ...
    d_evaluation = calibrate_against_frozen_r(ctx, graded, d_support, r_current, strengths)
    d_classified = classify_handles(d_evaluation["accepted"], graded, args.graded_threshold)
    ...
    # Repeatedly refine D while an earlier graded handle is certified.
    for round_index in range(args.max_d_refinements):
        d_sites = tuple(site for site in find_earlier_sites(ctx.sites, d_current["weights"]) if site.site_id not in r_weights)
        if not d_sites:
            break
        print(f"[4] D refinement {round_index}: {len(d_sites)} upstream candidates", flush=True)
        stage_ranking = rank_against_frozen_d(ctx, graded, d_sites, d_current)
        stage_support = get_support(stage_ranking["selector"]["ranked"], args.top_n, args.mass_fraction)
        stage_evaluation = calibrate_against_frozen_d(ctx, graded, stage_support, d_current, r_weights, r_readout, strengths)
        stage_classified = classify_handles(stage_evaluation["accepted"], graded, args.graded_threshold)
        candidate = choose_d(stage_classified)
        stage = {"round": round_index, "candidate_ids": [site.site_id for site in d_sites], **stage_ranking, "effective_support": stage_support, **stage_evaluation, "classified_handles": [public_handle(row) for row in stage_classified]}
        atomic_json(args.out_dir / f"D_refinement_{round_index}.json", stage)
