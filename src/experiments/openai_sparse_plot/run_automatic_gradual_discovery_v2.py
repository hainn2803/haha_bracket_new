...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mass-fraction", type=float, default=0.5)
    ...
    parser.add_argument("--max-d-refinements", type=int, default=3)
...
def fit_r(examples: Sequence[RediscoveryExample], runs: Mapping[str, ClampedRun], weights: Mapping[str, float]) -> tuple[Any, dict[str, float]]:
...
def fit_d(examples: Sequence[RediscoveryExample], runs: Mapping[str, ClampedRun], weights: Mapping[str, float]) -> tuple[Any, dict[str, dict[str, float]]]:
    fit_rows = [row for row in examples if row.split == "Dfit"]
...
def is_graded_d(metrics: Mapping[str, Mapping[str, float]], threshold: float) -> bool:
...
def classify_handles(rows: Sequence[Mapping[str, Any]], graded: Data, threshold: float) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if key in seen:
            continue
        seen.add(key)
        r_readout, r_accuracy = fit_r(graded.examples, graded.runs, weights)
        d_decoder, d_metrics = fit_d(graded.examples, graded.runs, weights)
        result.append({"row": dict(row), "weights": weights, "r_readout": r_readout, "r_accuracy": r_accuracy, "d_decoder": d_decoder, "d_metrics": d_metrics, "is_D": is_graded_d(d_metrics, threshold)})
    return result
...
# Stage 1: X -> R -> Y.
def run_coarse(ctx: Context, data: Data, strengths: Sequence[float]) -> dict[str, Any]:
...
# Stage 2: freeze R_late and test every earlier site in the coarse R candidate pool.
def find_earliest_r(ctx: Context, coarse: Data, graded: Data, current: Mapping[str, Any], parent_support: Sequence[Mapping[str, Any]], strengths: Sequence[float]) -> dict[str, Any] | None:
    earlier = [row for row in parent_support if str(row["site_id"]) not in current["weights"] and layer_order(str(row["site_id"])) < handle_order(current["weights"])]
    if not earlier:
        return None
    print(f"[2] Earliest R search: freeze {list(current['weights'])}", flush=True)

    # Dfit: patch each earlier site and measure both frozen R and Y.
    candidates = tuple(ctx.site_lookup[str(row["site_id"])] for row in earlier)
    probes = get_probe_sites(ctx, current["weights"])
    probe_ids = tuple(site.site_id for site in probes)
    singletons = tuple(HandleConfiguration(site.site_id, {site.site_id: 1.0}, 1.0) for site in candidates)
    margins, values = run_probes(ctx, coarse, singletons, "Dfit", probes)
    neural = {config.handle_id: neural_signature(coarse, "Dfit", margins[index], combine_probes(values[index], probe_ids, current["weights"]), current["weights"], current["r_readout"].orientation, True) for index, config in enumerate(singletons)}
    abstract, neural, scales = normalize_blocks(repeated_r_signature(coarse, "Dfit", 2), neural, coarse.pairs["Dfit"], 2)
    selector = match_signatures(abstract, neural, epsilon=ctx.args.selector_epsilon, beta=ctx.args.selector_beta)
    candidate_pool = selector["ranked"]

    # Dcal: test every singleton and pair in this small candidate pool.
    configs, metadata = make_calibration_configs(candidate_pool, strengths)
    cal_margins, cal_probes = run_probes(ctx, coarse, configs, "Dcal", probes)
    downstream = combine_probes(cal_probes, probe_ids, current["weights"])
    grid = [{**row, "summary": r_summary(coarse, "Dcal", cal_margins[index], downstream[index], current["weights"], current["r_readout"])} for index, row in enumerate(metadata)]
    best, tier = get_best_tier(grid)
    calibrated = best_strength_per_handle(grid)

    if not calibrated:
        return {"frozen_downstream": public_handle(current), "candidate_pool": candidate_pool, "normalization_scales": scales, "selector": selector, "calibration_best": best, "calibration_best_tier": tier, "calibrated_handles": [], "heldout": [], "classified": []}

    # Dte: test every Dcal-passing structural handle once, using its Dcal-selected strength.
    test_configs = tuple(HandleConfiguration(row["handle_id"], row["weights"], row["strength"]) for row in calibrated)
    test_margins, test_probes = run_probes(ctx, coarse, test_configs, "Dte", probes)
    restored_margins, _ = run_probes(ctx, coarse, test_configs, "Dte", probes, probe_ids)
    downstream = combine_probes(test_probes, probe_ids, current["weights"])
    heldout = []
    for index, row in enumerate(calibrated):
        summary = r_summary(coarse, "Dte", test_margins[index], downstream[index], current["weights"], current["r_readout"])
        mediation = mediation_summary(coarse.pairs["Dte"], coarse.by_id, coarse.runs, test_margins[index], restored_margins[index])
        heldout.append({**row, "heldout": summary, "mediation": mediation, "accepted": summary["passes"] and mediation["passes"]})

    accepted = [row for row in heldout if row["accepted"]]
    classified = classify_handles(accepted, graded, ctx.args.graded_threshold) if accepted else []
    return {"frozen_downstream": public_handle(current), "candidate_pool": candidate_pool, "normalization_scales": scales, "selector": selector, "calibration_best": best, "calibration_best_tier": tier, "calibrated_handles": calibrated, "heldout": heldout, "classified": classified}


# Stage 3: freeze the final R handle and start a new search for D.
def discover_d_from_r(ctx: Context, graded: Data, r_handle: Mapping[str, Any], strengths: Sequence[float]) -> dict[str, Any]:
    print(f"[3] D discovery: freeze R at {list(r_handle['weights'])}", flush=True)
    r_weights = r_handle["weights"]
    r_readout = r_handle["r_readout"]
    probes = get_probe_sites(ctx, r_weights)
    probe_ids = tuple(site.site_id for site in probes)

    # Dfit audit: intervene on every remaining site and measure frozen R and Y.
    # The downstream abstract effect is delta R, because D affects R through R=1[D>=2].
    # Exact graded D is checked later with the active-depth decoder.
    audit_sites = tuple(site for site in ctx.sites if site.site_id not in r_weights)
    audit_configs = tuple(HandleConfiguration(site.site_id, {site.site_id: 1.0}, 1.0) for site in audit_sites)
    fit_margins, fit_probes = run_probes(ctx, graded, audit_configs, "Dfit", probes)
    neural = {config.handle_id: neural_signature(graded, "Dfit", fit_margins[index], combine_probes(fit_probes[index], probe_ids, r_weights), r_weights, r_readout.orientation, True) for index, config in enumerate(audit_configs)}
    abstract, neural, scales = normalize_blocks(repeated_r_signature(graded, "Dfit", 2), neural, graded.pairs["Dfit"], 2)
    audit_selector = match_signatures(abstract, neural, epsilon=ctx.args.selector_epsilon, beta=ctx.args.selector_beta)

    # Primary D search: only sites causally upstream of the frozen R handle are eligible.
    upstream = upstream_sites(ctx.sites, tuple(r_weights))
    upstream_ids = tuple(site.site_id for site in upstream if site.site_id not in r_weights)
    if not upstream_ids:
        raise RuntimeError("no site is upstream of the frozen R handle")
    selector = match_signatures(abstract, {site_id: neural[site_id] for site_id in upstream_ids}, epsilon=ctx.args.selector_epsilon, beta=ctx.args.selector_beta)
    support = get_support(selector["ranked"], ctx.args.top_n, ctx.args.mass_fraction)

    # Dcal: test every singleton and pair for its causal effect on frozen R and Y.
    configs, metadata = make_calibration_configs(support, strengths)
    cal_margins, cal_probes = run_probes(ctx, graded, configs, "Dcal", probes)
    downstream = combine_probes(cal_probes, probe_ids, r_weights)
    grid = [{**row, "summary": r_summary(graded, "Dcal", cal_margins[index], downstream[index], r_weights, r_readout)} for index, row in enumerate(metadata)]
    best, tier = get_best_tier(grid)

    # Dte: validate the best tier and restore R to test mediation.
    test_configs = tuple(HandleConfiguration(row["handle_id"], row["weights"], row["strength"]) for row in tier)
    test_margins, test_probes = run_probes(ctx, graded, test_configs, "Dte", probes)
    restored_margins, _ = run_probes(ctx, graded, test_configs, "Dte", probes, probe_ids)
    downstream = combine_probes(test_probes, probe_ids, r_weights)
    heldout = []
    for index, row in enumerate(tier):
        summary = r_summary(graded, "Dte", test_margins[index], downstream[index], r_weights, r_readout)
        mediation = mediation_summary(graded.pairs["Dte"], graded.by_id, graded.runs, test_margins[index], restored_margins[index])
        heldout.append({**row, "heldout": summary, "mediation": mediation, "accepted": summary["passes"] and mediation["passes"]})

    accepted = [row for row in heldout if row["accepted"]]
    classified = classify_handles(accepted, graded, ctx.args.graded_threshold) if accepted else []
    return {"frozen_R": public_handle(r_handle), "candidate_count_audit": len(audit_sites), "candidate_count_upstream": len(upstream_ids), "normalization_scales": scales, "audit_selector": audit_selector, "selector": selector, "effective_support": support, "calibration_best": best, "calibration_best_tier": tier, "heldout_tier": heldout, "classified": classified}


def validate_d(ctx: Context, graded: Data, d_handle: Mapping[str, Any], r_handle: Mapping[str, Any]) -> dict[str, Any]:
    d_weights = d_handle["weights"]
    r_weights = r_handle["weights"]
    ...
    config = HandleConfiguration("direct_D", d_weights, float(d_handle["row"]["strength"]))
    probes = get_probe_sites(ctx, d_weights, r_weights)
    probe_ids = tuple(site.site_id for site in probes)
    splits = {}
    for split in ("Dcal", "Dte"):
        margins, values = run_probes(ctx, graded, (config,), split, probes)
        restored_margins, _ = run_probes(ctx, graded, (config,), split, probes, tuple(r_weights))
        summary = d_summary(graded, split, margins[0], values[0], probe_ids, d_weights, d_decoder, r_weights, r_readout)
        mediation = mediation_summary(graded.pairs[split], graded.by_id, graded.runs, margins[0], restored_margins[0])
        splits[split] = {"summary": summary, "R_mediation": mediation, "accepted": summary["passes"] and mediation["passes"]}
    return {"weights": d_weights, "decoder": d_decoder, "metrics": d_metrics, "R_readout": r_readout, "R_accuracy": r_accuracy, "splits": splits}


# Stage 4: freeze D_late and search for an earlier D.
def refine_d_once(ctx: Context, graded: Data, current: Mapping[str, Any], r_weights: Mapping[str, float], r_readout: Any, strengths: Sequence[float], round_index: int) -> dict[str, Any] | None:
    d_weights = current["weights"]
    d_decoder, _ = fit_d(graded.examples, graded.runs, d_weights)
    candidates = tuple(site for site in upstream_sites(ctx.sites, tuple(d_weights)) if site.site_id not in d_weights and site.site_id not in r_weights)
    if not candidates:
        return None
    print(f"[4] D refinement {round_index}: freeze {list(d_weights)}", flush=True)

    # Dfit: patch each upstream site and measure the decoded frozen D handle.
    d_probes = get_probe_sites(ctx, d_weights)
    d_probe_ids = tuple(site.site_id for site in d_probes)
    singletons = tuple(HandleConfiguration(site.site_id, {site.site_id: 1.0}, 1.0) for site in candidates)
    margins, values = run_probes(ctx, graded, singletons, "Dfit", d_probes)
    neural = {config.handle_id: neural_signature(graded, "Dfit", margins[index], combine_probes(values[index], d_probe_ids, d_weights), d_weights, d_decoder.slope, False) for index, config in enumerate(singletons)}
    abstract = abstract_e_signature(graded.pairs["Dfit"], graded.by_id, definition="active_depth")
    abstract, neural, scales = normalize_blocks(abstract, neural, graded.pairs["Dfit"], 1)
    selector = match_signatures(abstract, neural, epsilon=ctx.args.selector_epsilon, beta=ctx.args.selector_beta)
    support = get_support(selector["ranked"], ctx.args.top_n, ctx.args.mass_fraction)

    # Dcal: test whether each handle moves D, R, and Y as predicted.
    configs, metadata = make_calibration_configs(support, strengths)
    probes = get_probe_sites(ctx, d_weights, r_weights)
    probe_ids = tuple(site.site_id for site in probes)
    cal_margins, cal_probes = run_probes(ctx, graded, configs, "Dcal", probes)
    grid = [{**row, "summary": d_summary(graded, "Dcal", cal_margins[index], cal_probes[index], probe_ids, d_weights, d_decoder, r_weights, r_readout)} for index, row in enumerate(metadata)]
    best, tier = get_best_tier(grid)

    # Dte: validate the best tier and restore frozen D to test mediation.
    test_configs = tuple(HandleConfiguration(row["handle_id"], row["weights"], row["strength"]) for row in tier)
    test_margins, test_probes = run_probes(ctx, graded, test_configs, "Dte", probes)
    restored_margins, _ = run_probes(ctx, graded, test_configs, "Dte", probes, tuple(d_weights))
    heldout = []
    for index, row in enumerate(tier):
        summary = d_summary(graded, "Dte", test_margins[index], test_probes[index], probe_ids, d_weights, d_decoder, r_weights, r_readout)
        mediation = mediation_summary(graded.pairs["Dte"], graded.by_id, graded.runs, test_margins[index], restored_margins[index])
        heldout.append({**row, "heldout": summary, "mediation": mediation, "accepted": summary["passes"] and mediation["passes"]})

    accepted = [row for row in heldout if row["accepted"]]
    classified = classify_handles(accepted, graded, ctx.args.graded_threshold) if accepted else []
    return {"round": round_index, "frozen_D": public_handle(current), "candidate_count": len(candidates), "normalization_scales": scales, "selector": selector, "effective_support": support, "calibration_best": best, "calibration_best_tier": tier, "heldout_tier": heldout, "classified": classified}
...
def choose_d(handles: Sequence[Mapping[str, Any]]) -> Mapping[str, Any] | None:
    ...
    return max(choices, key=lambda row: (min(abs(float(row["d_metrics"][split]["pearson"])) for split in SPLITS), -int(row["row"]["k"]))) if choices else None
...
def main() -> None:
    ...
    coarse_result = run_coarse(ctx, coarse, strengths)
    ...
    r_refinement = find_earliest_r(ctx, coarse, graded, r_current, coarse_result["effective_support"], strengths)
    if r_refinement is not None:
        atomic_json(args.out_dir / "R_refinement.json", {key: value for key, value in r_refinement.items() if key != "classified"})
        r_candidate = choose_earliest_r(r_refinement["classified"])
    ...
    d_discovery = discover_d_from_r(ctx, graded, r_current, strengths)
    atomic_json(args.out_dir / "D_discovery.json", {key: value for key, value in d_discovery.items() if key != "classified"})
    d_current = choose_d(d_discovery["classified"])
    if d_current is None:
        raise RuntimeError("no graded D handle passed the independent upstream D search")

    direct_d = validate_d(ctx, graded, d_current, r_current)
    r_weights = r_current["weights"]
    r_readout = direct_d["R_readout"]
    d_chain = [{"name": "D", **public_handle(d_current), "source": "independent D discovery"}]

    for round_index in range(args.max_d_refinements):
        stage = refine_d_once(ctx, graded, d_current, r_weights, r_readout, strengths, round_index)
        if stage is None:
            break
        atomic_json(args.out_dir / f"D_refinement_{round_index}.json", {key: value for key, value in stage.items() if key != "classified"})
        candidate = choose_d(stage["classified"])
        if candidate is None or handle_order(candidate["weights"]) >= handle_order(d_current["weights"]):
            break
        d_current = candidate
        d_chain.append({"name": "D_early", **public_handle(d_current), "source": f"D refinement {round_index}"})
    ...
    result = {
        ...
        "rules": {"support": f"top {args.top_n}, mass >= {args.mass_fraction:g} * top-1", "calibration": "all singletons and pairs"},
        ...
        "R_chain": r_chain,
        "D_discovery": {key: value for key, value in d_discovery.items() if key != "classified"},
        "direct_D_to_R_to_Y": {"weights": direct_d["weights"], "decoder": direct_d["decoder"].to_dict(), "metrics": direct_d["metrics"], "R_readout": direct_d["R_readout"].to_dict(), "R_accuracy": direct_d["R_accuracy"], "splits": direct_d["splits"]},
        "D_chain": d_chain,
