...
def parse_args():
    # Configure signature-only discovery and held-out causal evaluation.
...
def construct_candidate_handles(ctx, pool, strengths, signature_bank, frozen_chain):
    # Build and rank candidate handles without using recovery or restoration tests.
    if not pool:
...
def main():
    # Discover a U chain from Y backward, then evaluate the graph only on Dte.
    args = parse_args()
    ...
    while True:
        ...
        ranked_handles, chosen, search_levels, handle_selector = construct_candidate_handles(ctx, candidate_pool, strengths, fit_bank, frozen_chain)
        ...
        print(
            ...
            f"readout_fit={u_accuracy['Dfit']:.4f}, readout_cal={u_accuracy['Dcal']:.4f}",
            flush=True,
        )
        round_index += 1
