...
def parse_args():
    parser = argparse.ArgumentParser(description="Ordered downstream graph discovery with conditional D invariance.")
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("outputs/automatic_gradual_discovery_v19"))
    ...
    parser.add_argument("--conditional-d-leakage-threshold", type=float, default=0.8)
...
def conditional_d_invariance(bank, weights, threshold):
    handle_deltas = []
    d_deltas = []

    for pair in bank.pairs:
        base = bank.by_id[pair.base_id]
        source = bank.by_id[pair.source_id]
        same_r = base.variable_value == source.variable_value
        base_d = discovery.d_value(base, "active_depth")
        source_d = discovery.d_value(source, "active_depth")

        if not same_r or abs(float(source_d) - float(base_d)) <= 1e-9:
            continue

        base_value = discovery.handle_value(bank.runs[pair.base_id], weights)
        source_value = discovery.handle_value(bank.runs[pair.source_id], weights)
        handle_deltas.append(float(source_value) - float(base_value))
        d_deltas.append(float(source_d) - float(base_d))

    pearson = None
    leakage = None
    passed = False

    if len(handle_deltas) >= 2 and float(np.std(d_deltas)) > 1e-12:
        if float(np.max(np.abs(handle_deltas))) <= 1e-12:
            pearson = 0.0
            leakage = 0.0
            passed = True
        else:
            pearson = float(np.corrcoef(handle_deltas, d_deltas)[0, 1])
            if np.isfinite(pearson):
                leakage = abs(pearson)
                passed = leakage <= float(threshold)

    print(
        f"bank={bank.name} "
        f"handle={list(weights.keys())} "
        f"records={len(handle_deltas)} "
        f"pearson={pearson} "
        f"passed={passed}",
        flush=True,
    )
...
def add_variable_metrics(handles, fit_bank, cal_bank, threshold):
    output = []

    for handle in handles:
        row = dict(handle)
        r_readout, r_accuracy = discovery.fit_r(fit_bank, cal_bank, row["weights"])
        d_decoder, d_metrics = discovery.fit_d(fit_bank, cal_bank, row["weights"])
        metrics = {}
        metrics[fit_bank.name] = conditional_d_invariance(fit_bank, row["weights"], threshold)
        metrics[cal_bank.name] = conditional_d_invariance(cal_bank, row["weights"], threshold)
        ...
        row["r_readout"] = r_readout
        row["r_accuracy"] = r_accuracy
        row["d_decoder"] = d_decoder
        row["d_metrics"] = d_metrics
        row["conditional_D_invariance"] = metrics
        row["conditional_D_invariance_passed"] = passed
        output.append(row)
...
def get_valid_handles(handles, variable, require_restoration=False):
    ...
    for row in handles:
        if not row["recovery_passed"]:
            continue
        if require_restoration and not row["restoration_passed"]:
            continue
        if variable == "R" and not row["conditional_D_invariance_passed"]:
            continue
        valid.append(row)
...
def build_handles(ctx, supports, pool, strengths, variable, fit_bank, cal_bank):
    ...
    fitted = add_variable_metrics(base_handles, fit_bank, cal_bank, ctx.args.conditional_d_leakage_threshold)
    handles = []
    for row in fitted:
...
def search_handles(ctx, pool, strengths, variable, fit_bank, metric_cal_bank, intervention_bank, r_handle=None, restore_handle=None, self_downstream=False):
    ...
    for size in range(1, ctx.args.max_handle_size + 1):
        ...
        valid_configurations = get_valid_handles(results, variable, require_restoration=False)
...
def build_downstream_graph(ctx, bank, nodes, configurations, terminal_name, variable, terminal_handle=None, r_handle=None):
    ...
    for source in reversed(nodes):
        ...
        for downstream in later_nodes:
            ...
            # check if {A, B} -> {A, C}. We don't want this case
...
def main():
    ...
    detailed = {
        "experiment": "automatic_gradual_discovery_v15",
        "config": config,
    ...
    summary = {"final_model": final_model, "passed": passed, "handles": handles, "R_edges": save_graph_edges(r_edges), "D_edges": save_graph_edges(d_edges), "detailed_output": "automatic_gradual_discovery_v15_detailed.json"}
    detailed_path = args.out_dir / "automatic_gradual_discovery_v15_detailed.json"
    summary_path = args.out_dir / "automatic_gradual_discovery_v15_summary.json"
